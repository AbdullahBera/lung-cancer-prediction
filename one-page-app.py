...
if st.session_state.page == 'Predictor-Dependent Analysis':
    ...
    prediction_percentage = model.predict_proba(input_df)[0][1] * 100  
...
# Non-Predictor Dependent Analysis Page
    ...
    with col1:
        ...
        fig_imp = px.bar(coef_df, x='Coefficient', y='Feature', orientation='h', height=350)  
        fig_imp.update_layout(
            xaxis=dict(title="Coefficient", titlefont=dict(size=14)),  
            yaxis=dict(title="Feature", titlefont=dict(size=14))  
        )
    ...
    with col2:
        ...
        roc_curve_fig.update_layout(
            xaxis=dict(title="False Positive Rate", titlefont=dict(size=14)),  
            yaxis=dict(title="True Positive Rate", titlefont=dict(size=14)),  
            height=350  
        )
    ...
    plt.xlabel('Number of Predictors', fontsize=10)  
    plt.ylabel('F1 Score', fontsize=10) 
    plt.title('F1 Score vs. Number of Predictors', fontsize=12)  
    plt.grid(True)
